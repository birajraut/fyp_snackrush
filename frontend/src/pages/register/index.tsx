...
const RegisterPage = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      password: Yup.string().min(6, 'Password must be at least 6 characters').max(20, 'Password must be 20 characters or less').required('Password is required'),
      address: Yup.string().max(100, 'Address must be 100 characters or less').required('Address is required'),
      ...
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-lg">
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" id="fullName" name="fullName" onChange={formik.handleChange} value={formik.values.fullName} className="border p-2 w-full" />
          </div>
          <div>
            <label htmlFor="email">Email Address</label>
            <input type="email" id="email" name="email" onChange={formik.handleChange} value={formik.values.email} className="border p-2 w-full" />
          </div>
          <div>
            <label htmlFor="password">Password</label>
            <input type="password" id="password" name="password" onChange={formik.handleChange} value={formik.values.password} className="border p-2 w-full" />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea id="address" name="address" onChange={formik.handleChange} value={formik.values.address} className="border p-2 w-full"></textarea>
          </div>

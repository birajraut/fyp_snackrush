...
//     addMarker(productLocation, 'red', 'Restaurant Location');
...
//     let index = 0;
//     setDeliveryLocation(routeCoords[1]);
...
//         return;
//       }
//           console.log('Route coordinates updated:', routeCoords);
...
//       }
//     };
//   }, [routeCoords?.length, deliveryLocation]);
...
const TrackProductPage = () => {
  ...
  const getRestaurantLocation = async () => {
    try {
      const res = await resturantDetailsFn(
        order?.products[0]?.product?.restaurant_id || order?.products[0]?.product_id?.restaurant_id
      );
  ...
  useEffect(() => {
    if (order) getRestaurantLocation();
  }, [order]);
  ...
  useEffect(() => {
    ...
    const addMarker = (coords: Coordinates, color: string, label: string) => {
      return new mapboxgl.Marker({ color })
        .setLngLat(coords)
        .setPopup(new mapboxgl.Popup().setText(label))
        .addTo(map.current!);
    ...
    if (deliveryLocation) {
      deliveryMarkerRef.current = addMarker(deliveryLocation, DELIVERY_MARKER_COLOR, 'Delivery Person');
    }

    const fetchAndDisplayRoute = async () => {
      try {
        const response = await fetch(
          `https://api.mapbox.com/directions/v5/mapbox/driving/${productLocation[0]},${productLocation[1]};${userLocation[0]},${userLocation[1]}?geometries=geojson&access_token=${mapboxgl.accessToken}`
        );
        const data = await response.json();
    ...
    map.current.on('load', fetchAndDisplayRoute);
  ...
  // Dummy delivery movement
  useEffect(() => {
    if (!routeCoords.length) return;
if(state?.order?.delivery_status!=='On the way' ) return
    let index = 0;
  ...
  useEffect(() => {
    if (!deliveryLocation || !map.current) return;

    if (!deliveryMarkerRef.current) {
      deliveryMarkerRef.current = new mapboxgl.Marker({ color: DELIVERY_MARKER_COLOR })
    ...
    if (Math.random() > 0.7) {
      map.current.flyTo({ center: deliveryLocation, essential: true });
    }
  }, [deliveryLocation]);
